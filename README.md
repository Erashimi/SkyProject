...
**Содержит декораторы для выполнения функций**

1. Декоратор `log`

Автоматически логирует успешные выполнения функций и ошибки, формат логов:
- При успехе: `{имя_функции} ok`.
- При ошибке: `{имя_функции} error: {тип_ошибки}. Inputs: {аргументы}`.

`filename` (опциональный):  
  Если указан — логи записываются в файл.  
...
Отсортировать по дате? Да
Отсортировать по возрастанию или по убыванию?  по возрастанию
Выводить только рублевые тразакции? Нет
Отфильтровать список транзакций по определенному слову 
в описании? Да
Введите слово: PayPal

Всего банковских операций в выборке: 3

12.03.2022 Перевод 
Visa 1234 56** **** 9012 -> Счет **4321
Сумма: 5000 RUB
